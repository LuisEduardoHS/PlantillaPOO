...
int startGameEngine(void *ptrMsg){
    ...
    scale = glm::vec3(0.0025f, 0.0025f, 0.0025f);	// it's a bit too big for our scene, so scale it down
    model->setScale(&scale);
    model->setTranslate(&translate);
    try {
        std::vector<Animation> idleAnimation = Animation::loadAllAnimations("models/BaseSpiderman/Idle.fbx", model->GetBoneInfoMap(), model->getBonesInfo(), model->GetBoneCount());
        std::vector<Animation> walkingAnimation = Animation::loadAllAnimations("models/BaseSpiderman/Walking.fbx", model->GetBoneInfoMap(), model->getBonesInfo(), model->GetBoneCount());
        std::vector<Animation> jumpingAnimation = Animation::loadAllAnimations("models/BaseSpiderman/Jumping.fbx", model->GetBoneInfoMap(), model->getBonesInfo(), model->GetBoneCount());
		std::vector<Animation> walkingBackAnimation = Animation::loadAllAnimations("models/BaseSpiderman/Walking Backwards.fbx", model->GetBoneInfoMap(), model->getBonesInfo(), model->GetBoneCount());
        std::vector<Animation> leftStrafeAnimation = Animation::loadAllAnimations("models/BaseSpiderman/Left Strafe Walking.fbx", model->GetBoneInfoMap(), model->getBonesInfo(), model->GetBoneCount());
        std::vector<Animation> rightStrafeAnimation = Animation::loadAllAnimations("models/BaseSpiderman/Right Strafe Walking.fbx", model->GetBoneInfoMap(), model->getBonesInfo(), model->GetBoneCount());
        std::vector<Animation> runningAnimation = Animation::loadAllAnimations("models/BaseSpiderman/Running.fbx", model->GetBoneInfoMap(), model->getBonesInfo(), model->GetBoneCount());

		std::vector<Animation> allAnimations;

		allAnimations.insert(allAnimations.end(), idleAnimation.begin(), idleAnimation.end());
        allAnimations.insert(allAnimations.end(), walkingAnimation.begin(), walkingAnimation.end());
        allAnimations.insert(allAnimations.end(), jumpingAnimation.begin(), jumpingAnimation.end());
		allAnimations.insert(allAnimations.end(), walkingBackAnimation.begin(), walkingBackAnimation.end());
		allAnimations.insert(allAnimations.end(), leftStrafeAnimation.begin(), leftStrafeAnimation.end());
		allAnimations.insert(allAnimations.end(), rightStrafeAnimation.begin(), rightStrafeAnimation.end());
		allAnimations.insert(allAnimations.end(), runningAnimation.begin(), runningAnimation.end());
...
bool checkInput(GameActions *actions, Scene* scene) {
    ...
    // Conectar animaciones con eventos
    Model* OGLobj = scene->getMainModel();
    if (*actions->jump > 0) {
        OGLobj->setAnimation(2); // Jumping
    }
    else if(actions->sprintPressed && actions->advance > 0) {
        OGLobj->setAnimation(6); // Running
	}
    else if (actions->advance > 0) {
        OGLobj->setAnimation(1); // Walking
    }
    else if (actions->advance < 0) {
		OGLobj->setAnimation(3); // Walking Backwards
    }
    else if (actions->sideAdvance > 0){
		OGLobj->setAnimation(4); // Left Strafe
    }
    else if (actions->sideAdvance < 0) {
		OGLobj->setAnimation(5); // Right Strafe
    }
    else {
        OGLobj->setAnimation(0); // Idle por defecto
    }
...
//        pos.y = *actions->jump > 0 ? pos.y : scene->getTerreno()->Superficie(pos.x, pos.z);
    ...
    if (actions->advance != 0) {
        glm::vec3 pos = *OGLobj->getTranslate();
        float speedMultiplier = actions->sprintPressed ? 2.0f : 1.0f;  // Duplica velocidad si sprint
        pos.x += actions->advance * (3 * gameTime.deltaTime / 100) * speedMultiplier * glm::sin(glm::radians(OGLobj->getRotY()));
        pos.z += actions->advance * (3 * gameTime.deltaTime / 100) * speedMultiplier * glm::cos(glm::radians(OGLobj->getRotY()));
        // Posicionamos la camara/modelo pixeles arriba de su posicion en el terreno
//        pos.y = *actions->jump > 0 ? pos.y : scene->getTerreno()->Superficie(pos.x, pos.z);
        OGLobj->setNextTranslate(&pos);
        //OGLobj->setAnimation(1); // walk
    }
    if (*actions->jump > 0){
